...
class PhoneCallLogView extends StatefulWidget {
  const PhoneCallLogView({super.key});
  @override
  State<PhoneCallLogView> createState() => _PhoneCallLogViewState();
}

class _PhoneCallLogViewState extends State<PhoneCallLogView> {
  AdministrationController get _c => Get.find<AdministrationController>();

  static final _tabs = [
    const AdminTabItem(
        label: 'Visitor Book', route: AppRoutes.adminVisitorBook),
    const AdminTabItem(label: 'Complaint', route: AppRoutes.adminComplaint),
    const AdminTabItem(
        label: 'Phone Call Log',
        route: AppRoutes.adminPhoneCallLog,
        isActive: true),
    const AdminTabItem(
        label: 'Postal Dispatch', route: AppRoutes.adminPostalDispatch),
    const AdminTabItem(
        label: 'Postal Receive', route: AppRoutes.adminPostalReceive),
    const AdminTabItem(label: 'Admin Setup', route: AppRoutes.adminSetup),
    const AdminTabItem(
        label: 'Admission Query', route: AppRoutes.adminAdmissionQuery),
    ...
    return Obx(() {
      ...
      return RefreshIndicator(
        ...
        child: ListView.builder(
          ...
          itemBuilder: (_, i) {
            final p = items[i];
            final isIn = p.callType == 'I';
            return AdminRecordCard(
              icon:
                  isIn ? Icons.call_received_rounded : Icons.call_made_rounded,
              iconColor:
                  isIn ? const Color(0xFF059669) : const Color(0xFF0EA5E9),
              title: p.name.isNotEmpty ? p.name : p.phone,
              subtitle:
                  '${p.phone} · ${p.callTypeLabel}${p.date.isNotEmpty ? ' · ${p.date}' : ''}',
              onEdit: () {
                _c.startEditPhoneCall(p);
                _showForm(context);
              },
              onDelete: () => _confirmDelete(
                  'Delete call log for "${p.phone}"?',
                  () => _c.deletePhoneCall(p.id)),
              extraBadges: [
                _badge(p.callTypeLabel,
                    isIn ? const Color(0xFF059669) : const Color(0xFF0EA5E9)),
                if (p.callDuration.isNotEmpty)
                  _badge(p.callDuration, const Color(0xFF7C3AED)),
                if (p.nextFollowUpDate.isNotEmpty)
                  _badge(
                      'Follow: ${p.nextFollowUpDate}', const Color(0xFFD97706)),
              ],
            );
          },
        ),
  ...
  void _showForm(BuildContext ctx) {
    showModalBottomSheet(
      ...
      builder: (sheetCtx) => _FormSheet(
        title:
            _c.editingId.value != null ? 'Edit Phone Call' : 'Log Phone Call',
        onClose: () {
          _c.resetPhoneCallForm();
        ...
        child: Column(children: [
          ...
          AdminField(controller: _c.callNameCtrl, label: 'Contact Name'),
          AdminField(
              controller: _c.callPhoneCtrl,
              label: 'Phone',
              required: true,
              keyboardType: TextInputType.phone),
          _section('Call Details'),
          // Call type chip selector
          Text('Call Type',
          ...
          Obx(() => Row(children: [
                _typeChip('Incoming', 'I'),
                const SizedBox(width: 10),
                _typeChip('Outgoing', 'O'),
              ])),
          const SizedBox(height: 14),
          AdminField(controller: _c.callDateCtrl, label: 'Date (YYYY-MM-DD)'),
          AdminField(
              controller: _c.callDurationCtrl, label: 'Duration (e.g. 5 min)'),
          AdminField(
              controller: _c.followUpDateCtrl,
              label: 'Follow Up Date (YYYY-MM-DD)'),
          AdminField(
              controller: _c.callDescCtrl, label: 'Description', maxLines: 3),
          const SizedBox(height: 8),
          Obx(() => _actionRow(
                isSaving: _c.isSaving.value,
                isEditing: _c.editingId.value != null,
                onSave: () async {
                  await _c.savePhoneCall();
  ...
  Widget _typeChip(String label, String value) => Obx(() {
        final isSelected = _c.callType.value == value;
        return GestureDetector(
          onTap: () => _c.callType.value = value,
