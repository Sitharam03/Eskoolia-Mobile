...
class InventoryCategoryView extends StatelessWidget {
  const InventoryCategoryView({super.key});

  InventoryCategoryController get _c =>
  ...
  Widget build(BuildContext context) {
    return AppScaffold(
      ...
      body: Column(children: [
        const InventoryNavTabs(activeRoute: AppRoutes.inventoryCategories),
        Expanded(
          child: Obx(() {
...
class _StatsRow extends StatelessWidget {
  final InventoryCategoryController c;
  const _StatsRow({required this.c});

  @override
  Widget build(BuildContext context) {
    return Obx(() {
      final total = c.categories.length;
      final active = c.categories.where((cat) => cat.isActive).length;
      return Row(children: [
        _StatTile(
            value: '$total',
            label: 'Total',
      ...
      child: Obx(() => Column(children: [
            ...
            Padding(
              ...
              child:
                ...
                sFieldLabel('Category Title *'),
                const SizedBox(height: 6),
                sTextField(
                    controller: c.titleCtrl, hint: 'e.g. Stationery'),
                const SizedBox(height: 14),
...
class _CategoryList extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Obx(() {
      final items = c.filteredCategories;
      if (items.isEmpty) {
        return sEmptyState('No categories found', Icons.category_outlined);
      }
      return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        _ListHeader(title: 'Categories', count: items.length),
        const SizedBox(height: 10),
        ...items.map((cat) => _CategoryCard(
              category: cat,
              onEdit: () => c.startEdit(cat),
              onDelete: () => showDialog(
                  context: context,
                  builder: (_) => sDeleteDialog(
                      context: context,
                      message: 'Delete category "${cat.title}"?',
